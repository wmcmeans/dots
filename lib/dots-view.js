...
View.prototype.beginMove = function (event) {
  ...
  function addDraggableLineToHMTL() {
    // find position of last selected spot in chain
    var lastDot = board.selectedDots[board.selectedDots.length - 1];
    var lastDotId = lastDot.pos[0] + "-" + lastDot.pos[1];
    var lastDotPos = $(lastDotId).position();

    // debugger
    var div = jsPlumb.getElement(lastDotId);
...
View.prototype.render = function () {
  this.addjsPlumbEndPoints();
  this.updateScore();
  this.updateClasses();
};

View.prototype.addjsPlumbEndPoints = function () {
  return;
};
